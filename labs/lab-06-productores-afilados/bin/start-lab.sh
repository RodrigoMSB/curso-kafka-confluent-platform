...
echo -e "    Test de throughput:      ${BOLD}kafka-cli/perf-test.sh <TOPIC> N [opciones]${NC}"
echo -e "    Productor naive:         ${BOLD}kafka-cli/produce-naive.sh <TOPIC> N${NC}"
echo -e "    Productor idempotente:   ${BOLD}kafka-cli/produce-idempotent.sh <TOPIC> N${NC}"
echo -e "    Productor transaccional: ${BOLD}kafka-cli/produce-transactional.sh N${NC}"
echo -e "    Consumir aislado:        ${BOLD}kafka-cli/consume-isolated.sh <TOPIC> <ISOLATION>${NC}"
echo -e "    Detener laboratorio:     ${BOLD}bin/stop-lab.sh${NC}"
echo ""
echo -e "${YELLOW}  Abre la guía: guia/01-tuning-batch-y-linger.md${NC}"
echo ""
